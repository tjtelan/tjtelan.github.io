...
This is a summary of my experience with writing a Rust DB-backed server/client with gRPC to communicate to the backend, and Diesel as an ORM to be used with PostgreSQL.
...
Based on Kevin Hoffman's experience, and the download activity on crates.io, I also used Pingcap's library [grpc-rs](https://github.com/pingcap/grpc-rs).
...
User input -> Create Diesel struct + any data manipulation -> Convert Diesel struct into Proto struct -> Send Proto struct in gRPC call

Inserts into the database - Server side:

Receive Proto struct -> Convert Proto struct into Diesel struct + Any data manipulation -> Insert into DB
...
The Diesel tests were very useful helping me figure it out. https://github.com/diesel-rs/diesel/blob/v1.3.1/diesel_tests/tests/custom_types.rs 
...
First thing I need my database schema

[schema.rs](https://github.com/tjtelan/rust-examples/blob/master/cli_grpc_diesel/workspace/models/schema.rs)
...
Corresponding proto message definition
...
##### Convert business logic struct to/from proto-generated struct
...
Here we see that I convert the proto-form struct `req` into the business logic form `OrderForm` by calling `.into()`. Since the `create_order()` impl takes in `OrderForm`, there is no need to annotate the type with `.into()` and we’re able to stay focused
...
And here we take the business logic form `order_form` and use it to create the insertable struct `new_order` with all of the column values for Diesel to execute.

##### Do it again, in reverse, for queries

Last was repeating all of this work, but for making queries.

This ended up being slightly off pattern, because I was returning a list, and the From trait apparently is not easily implemented for a Vec. But I ended up getting lazy, and wrapping this in a function that loops and uses my already implemented From traits on the individual types.
